...
a socket as an object that can throw events. 

The basic events associated to a socket are READ and WRITE. So, why not having a socket class that has the 
features of the Dispatcher class? 

That is what the class defined in untwisted.network.SuperSocket is. The SuperSocket class 
abstracts the behavior of other classes. 

The SuperSocket class holds a file descriptor that is associated with objects that can be associated with a file descriptor. 
When a SuperSocket instance is created it registers itself in the untwisted reactor. 
